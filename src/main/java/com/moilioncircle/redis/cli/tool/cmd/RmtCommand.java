...
public class RmtCommand extends AbstractCommand {
    ...
    private static final Option FROM = Option.builder("f").longOpt("from").required(false).hasArg().argName("uri").type(String.class).desc("Input uri. eg: redis://host:port?authPassword=foobar redis:///path/to/dump.rdb redis:///path/to/appendonly.aof.").build();
    ...
    private static final Option TO = Option.builder("t").longOpt("to").required(false).hasArg().argName("uri").type(String.class).desc("Output uri. eg: redis://host:port?authPassword=foobar.").build();
    ...
    protected void doExecute(CommandLine line) throws Exception {
        if (line.hasOption("help")) {
            ...
            if (!line.hasOption("in") && !line.hasOption("from")) {
                sb.append("i or f ");
            }
            ...
            String to = line.getOption("to");
            File input = line.getOption("in");
            String from = line.getOption("from");
            ...
            if (from == null && input != null) {
                URI u = input.toURI();
                from = new URI("redis", u.getRawAuthority(), u.getRawPath(), u.getRawQuery(), u.getRawFragment()).toString();
            }

            RedisURI uri = new RedisURI(to);
            if (uri.getFileType() != null) {
                writeLine("Invalid uri: " + to);
                return;
            }
            Replicator r = new RedisReplicator(from);
            dress(r, to, db, regexs, Type.parse(type), replace);
    ...
    private void dress(Replicator r, String to, List<Long> db, List<String> regexs, List<Type> types, boolean replace) throws Exception {
        r.setRdbVisitor(new MigrateRdbVisitor(r, to, db, regexs, types, replace));
