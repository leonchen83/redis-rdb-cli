...
public class RespRdbVisitor extends AbstractRdbVisitor {
    ...
    private void emit(byte[] command, byte[] key, byte[]... ary) throws IOException {
        ...
        out.write('\n');
        out.write(DOLLAR);
        escape.encode(String.valueOf(command.length).getBytes(), out);
        out.write('\r');
        out.write('\n');
        escape.encode(command, out);
        out.write('\r');
        out.write('\n');
        for (final byte[] arg : args) {
            out.write(DOLLAR);
            escape.encode(String.valueOf(arg.length).getBytes(), out);
            out.write('\r');
            out.write('\n');
            escape.encode(arg, out);
            out.write('\r');
            out.write('\n');
        }
        out.flush();
